...
an LLM, and never prompts for an API key during initialization (spec `20-harness-worker.md` §9.20).
...
(spec `20-harness-worker.md` §9.21). Uncertain domain classification enables the required `general` domain instead of
guessing.
...
users can accept, amend, or reject it before relying on it (spec `50-contracts-data.md` §9.22.6).
