...
### 9.47 GitHub event ingress and agent-authored PR attribution

The daemon is a local, NAT'd process and exposes no public endpoint. GitHub events (PR opened, updated, ready-for-review, commented; issue commented) reach it through two converging paths: an event-relay PUSH for low latency and a conditional POLL for completeness. Both feed the same ingress queue and the same kick-off evaluation as §9.36 — event ingestion accelerates kick-off and the §9.33.4 review pipeline; it never creates a second trigger authority, and a missed or delayed event never loses work (the §9.43 reconcile poll is the completeness authority, board-wins).

**(a) Push path — GitHub Actions over Tailscale.** A repo-trusted, operator-deployed GitHub Actions workflow triggers on the configured event set, joins the operator's tailnet as an EPHEMERAL node through the pinned `tailscale/github-action`, and POSTs an event envelope to the daemon's tailnet-only relay endpoint.

- **Trigger hygiene.** The workflow that holds the relay and tailnet credentials resolves its definition from the trusted default branch only — never from the PR's merge ref (`workflow_run`-driven, or an equivalent default-branch-resident pattern). This is mandatory because on plain `pull_request` triggers GitHub executes the workflow definition from the PR being built, and same-repo PRs run with repository-wide secrets: routine PR authors here are untrusted agents (§6.1), so an agent-authored PR MUST never be able to execute its own modified delivery workflow. Untrusted PR code is never checked out or executed on the runner; event payload content reaches the delivery step only as data (environment interpolation), never shell-inline. The action is pinned to an immutable revision (commit SHA) and the Tailscale client to an explicit version; the workflow grants minimal `permissions` for its writes (e.g. `contents: read`, `pull-requests: write`, `issues: write`) and keys `concurrency` per PR or issue so duplicate deliveries are bounded, not multiplied.
- **Tailnet hardening.** Relay credentials are an OAuth client scoped to the `auth_keys` capability only, bound to a dedicated runner tag; the tailnet ACL grants that runner tag reach exclusively to the daemon's node tag on the relay port (deny-by-default, with accept/deny tests alongside the grant); the daemon node's tag is admin-owned so a relay credential can never re-tag or escalate to it; runner nodes are ephemeral and logged out at job end — no long-lived runner credentials exist on either side. Public ingress paths are rejected by design: a public webhook URL defeats the local-first/NAT premise, and outbound-serving relays (e.g. Tailscale Funnel — beta, port-locked, unauthenticated by construction) re-create the public endpoint the architecture exists to avoid.
- **The tunnel is not authentication.** Envelope authentication — relay-credential validation and the accept/deny decision on every envelope — is performed by Arbitraitor policy evaluation (§2.2, §16.1), never by an Orchestraitor-implemented check: the relay endpoint is an Arbitraitor-mediated transport surface that makes no independent allow/deny or trust decision, exactly like the §9.38 MCP proxy; the tailnet node identity is defense-in-depth input to that evaluation, never a sole authentication factor. Per §16.2 the ingress path ships only once the corresponding Arbitraitor capability for ingress-envelope authentication exists — until then the relay endpoint is `blocked:arbitraitor` and fails closed. An envelope is untrusted content (§6.1): its payload is treated as data — marker-wrapped and passed through the same untrusted-content boundary as agent-filed issue bodies (§9.37(d)) before any agent context observes it — and kick-off decisions are computed from trusted configuration and board state, never envelope-provided instructions; accepted deliveries produce Arbitraitor receipts recorded in the event store (§9.17).
- **Credential storage.** The relay credential resolves through the §9.23 secret URIs (`secret://keyring/…`, `secret://env/…`) into `secrecy::SecretString` and never appears in plaintext configuration (§9.23.1); its lifetime is bounded and operator-configured, it is revocable without redeploying the daemon, and errors never surface its value (§9.23.4). The runner side stores it as a GitHub Actions environment secret.
- **Idempotency.** Delivery is at-least-once (GitHub retries, Actions re-runs, manual redelivery): the daemon deduplicates on a durable delivery key — the platform delivery identifier (webhook `X-GitHub-Delivery` GUID; for the Actions push path the run ID plus attempt number) plus repository, event type, and head SHA — with a redelivery window long enough to cover the platform's redelivery/retention horizon (30 days), and a redelivery never spawns a second campaign for the same item (§9.26.3). The ingress queue is bounded: sustained flooding (authenticated credential holders included) drops surplus deliveries and records the `degraded` marker — never unbounded queue growth, the §7.3 resource-exhaustion DoS class the bound exists to prevent.
- **Failure behavior.** A configured push path that cannot deliver within a configured window surfaces `degraded` through the daemon `health` report and a stored event, emits a needs-human signal (§9.33.4), and the poll path keeps the loop complete — never silent degradation, never unbounded retry (§9.26), never automatic widening of the poll cadence beyond rate-limit feedback. `degraded` clears on the next successful relay delivery for the delivery-failure case, and only after a flood-free window for the overflow case.

**(b) Poll fallback — conditional requests.** Without a tailnet — feature disabled, credentials absent, or the push path degraded — the GitHub board provider polls the REST API with conditional requests (ETag/304) and honors `X-Poll-Interval` and rate-limit feedback (§9.43). Polling is slower but complete over the same event filter, obeys the same provider rate budgets, and converges on the same ingress queue and dedup table as the push path; the two mechanisms are ordering-insensitive inputs to kick-off evaluation, not state authorities.

**(c) Agent-authored PR attribution labels.** When a PR is opened by a principal in the configured service-identity set (§9.41 — the `arbsec-agent` GitHub App service identity), the lifecycle records attribution as labels at creation time through the same Arbitraitor-mediated provider-write path the coordinator tools use (§9.39) — never a raw PR write: the operator-configurable `agent-created` label (the name is configuration) marks agent authorship, and a diff touching the security-sensitive change classes the workflow policy and §21.1 declare (privileged brokers, sandboxing, policy enforcement, capability issuance, filesystem projection, network controls, secret handling, `unsafe`; path mapping is §9.22 configuration) additionally carries `needs-human-review` from open time — before any reviewer selection. Labels derive only from launch facts (PR author equals the configured service identity; actual changed paths), never from model-generated text, and application is idempotent. Attribution labels are daemon-owned provenance — an explicit, named exception to §9.43 board-wins: the reconcile poll corrects them in BOTH directions from the same launch facts (re-applying one removed from a service-identity PR, removing one carried by a human-authored PR; each label converges to the set its launch facts imply — review-completion signaling lives in board Status, never in these labels), records each correction as a `board-diverged` event, and never rewrites any other label — attribution may be corrected, never fought silently.
