...
- No conflicting in-flight PR touching the same spec section / crate (AGENTS.md; spec `10-orchestrator.md` §9.33.2).
- The body has:
  - a **single, bounded objective**;
  - an explicit **specification reference** (`docs/spec/<file>.md §N` and/or `tech-stack.md §N`);
  - **acceptance criteria** that are observable and testable (spec `10-orchestrator.md` §9.33.2);
...
If any field is unset or a criterion is missing, the issue stays `Backlog`, not `Ready`. Never guess a value to force `Ready` — escalate to the project-manager (spec `10-orchestrator.md` §9.33.1).

## Definition of Done (a PR may merge)

A PR may merge only when ALL of (spec `10-orchestrator.md` §9.33.4, AGENTS.md "Review and merge invariants"):

- All required and non-optional CI checks pass against the **current HEAD**. A missing/skipped check is a failure, not a pass (spec `50-contracts-data.md` §21.10).
...
Reaching a configured `max_review_loops`, cost, or elapsed-time limit produces a `blocked` or `needs-human` state (spec `10-orchestrator.md` §9.24, §9.33.4). It **never** counts as successful convergence. The implementer adds a human reviewer and moves on; the issue/PR is not silently approved.
